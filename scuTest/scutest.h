...
SAMP_BOOLEAN AddInstancesToFile(STORAGE_OPTIONS options, InstanceNode** instanceList, char fname[]);
...
int ImageSentCount(InstanceNode* node);
...
void  CheckcmdArgsImageDetails(char* A_argv[], int i, STORAGE_OPTIONS* A_options);
...
void update_list(InstanceNode** A_list, InstanceNode* newNode);
SAMP_BOOLEAN traverseimages(STORAGE_OPTIONS* A_options, InstanceNode** instanceList, char fname[]);
}